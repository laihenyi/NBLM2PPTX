...
> ✨ **更新 (2026-01-21)**: v2.2.1 版本发布 - 完整多语系改版！所有语言版本现采用专业浅色主题设计，改善用户体验并标准化文档。
...
### ⚡ 双模式 OCR 系统
- **Lite 模型（默认）**：使用 `gemini-2.5-flash-lite` 进行 OCR，速度快 40-50%，节省 50% API 额度
- **标准模型（可选）**：使用 `gemini-2.5-flash` 完整检测字体大小、粗细、颜色等样式
- **用户灵活选择**：在页面选择阶段可自由切换 OCR 模型，根据需求平衡速度与质量

### 🚀 并行处理技术
- **同步执行**：文字移除与 OCR 同时进行，无需等待
- **处理时间缩短**：从单页 3-4 秒降至 2-3 秒
- **智能容错**：单一 API 失败不影响整体流程，提升稳定性

### 💡 清晰的使用建议
- **Lite 模型适合**：纯文字笔记、会议记录、内容草稿（不在意视觉格式）
- **标准模型适合**：精美演示文稿、品牌展示、教学幻灯片（需保留视觉层次）
- **透明化限制说明**：明确告知 Lite 模型无法检测字体样式的限制
...
> **Lite 模型**：所有文字采用统一样式，无字体大小差异（速度快，省 API 额度）
> **标准模型**：完整保留标题与正文的字体大小层次（完整样式检测）
...
### 🌍 完整多语系改版
- **所有语言专业设计**：完全重新设计所有语言版本（英文、西班牙文、日文、法文、简体中文），从深色主题升级为现代浅色主题
- **统一字体系统**：迁移至 Poppins（标题）+ Open Sans（正文），配合语言特定后备字体（Noto Sans JP、Noto Sans SC 等）
- **专业蓝色配色方案**：在所有版本中实施一致的 #3B82F6 主色调，营造信任与专业感
- **增强的 API Key 模态框**：基于浏览器的 API Key 存储，集成 localStorage，无需编辑代码
- **可折叠 UI 元素**：添加可折叠的提醒横幅和工具区块，界面更简洁

### 📚 标准化文档
- **完整的 README**：所有语言的 README 文件现采用 204 行完整结构
- **快速入门指南**：添加 3 步快速入门说明，优化新手体验
- **免费 API 配额详情**：清楚说明 Google Gemini 免费层级（15 RPM, 1500 RPD, 无需信用卡）
- **完整 FAQ 章节**：5 组问答涵盖 API Key、安全性、失败处理、分享和离线使用等常见问题

### 🎨 设计系统更新
- **浅色主题**：#F8FAFC 背景提升可读性
- **现代卡片布局**：简洁边框（#E2E8F0）和细微阴影
- **专业 SVG 图标**：用专业 SVG 图形替换 emoji 图标
- **响应式排版**：优化所有屏幕尺寸的字体大小和间距
