...
### ⚡ デュアルモードOCRシステム
- **Liteモデル（デフォルト）**：`gemini-2.5-flash-lite`をOCRに使用、40-50%高速化、50%のAPIクォータ節約
- **標準モデル（オプション）**：`gemini-2.5-flash`を使用してフォントサイズ、太さ、色スタイルを完全検出
- **ユーザーの柔軟性**：ページ選択時にOCRモデルを切り替え、ニーズに応じて速度と品質のバランスを調整

### 🚀 並列処理技術
- **同時実行**：テキスト除去とOCRが同時に実行、待機なし
- **処理時間短縮**：ページあたり3-4秒から2-3秒に短縮
- **インテリジェント耐障害性**：単一のAPI障害が全体のワークフローに影響せず、安定性向上

### 💡 明確な使用ガイドライン
- **Liteモデルに最適**：プレーンテキストメモ、議事録、コンテンツドラフト（視覚的なフォーマットが重要でない場合）
- **標準モデルに最適**：美しいプレゼンテーション、ブランドショーケース、教育スライド（視覚的階層が必要）
- **透明な制限**：Liteモデルがフォントスタイルを検出できないことについての明確なコミュニケーション
...
> **Lite モデル**：すべてのテキストが統一スタイルで、フォントサイズの違いなし（高速、API クォータ節約）
> **標準モデル**：タイトルと本文のフォントサイズ階層を完全保持（完全なスタイル検出）
